...
def split_blocks(message: bytes, block_size: int = 128):
    return [message[i:i + block_size] for i in range(0, len(message), block_size)]


def sha512_custom(message: bytes) -> str:
    h_vals = INITIAL_HASHES.copy()
    msg = pad_message(message)
    ...
    for block in blocks:
        ...
        a, b, c, d, e, f, g, h = h_vals
        for i in range(80):
            T1 = (h + big_sigma1(e) + ch(e, f, g) + K[i] + W[i]) & MASK64
        ...
        h_vals = [(hv + val) & MASK64 for hv, val in zip(h_vals, [a, b, c, d, e, f, g, h])]

    return ''.join(f'{val:016x}' for val in h_vals)


@router.post("/sha512")
async def compute_sha512(req: HashRequest):
    msg_bytes = req.message.encode('utf-8')
    digest = sha512_custom(msg_bytes)
